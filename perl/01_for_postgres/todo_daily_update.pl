...
$todo_data   = $_TD->{new}{todo_name};
$period_data = $_TD->{new}{period};
@wday   = ($_TD->{new}{sun},
           $_TD->{new}{mon},
           $_TD->{new}{tue},
           $_TD->{new}{wed},
...
$query = sprintf("delete from %s where %s='%s' and %s='%s' and %s='%s';",
                 $table,
                 $todo, $todo_data, $parent, $parent_data, $child, $child_data);

$rv = spi_exec_query($query);

# period_date
$period_start = &strdate_to_timestamp($period_data, 'start');
$period_end   = &strdate_to_timestamp($period_data, 'end');
